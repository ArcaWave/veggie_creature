...
전시 현장 = 대부분 **아이가 종이에 그린 채소 그림 + (있으면) 스티커(모자·팔·다리)** — 스티커 없는 그림도 많다.
실제 채소를 보여줘도 된다. 스캔 스테이션이 사진에서 몸통 채소와 스티커 종류를 읽고, 같은 조합의 클레이
3D 캐릭터를 월드에 보낸다. (인식 프롬프트는 그림을 기준으로 쓴다 — "실제 채소"만 말하던 첫 프롬프트는
그림을 전부 "없음"으로 돌려보냈다.) 스티커가 없으면 모자 없음 + 나뭇가지 팔다리.
